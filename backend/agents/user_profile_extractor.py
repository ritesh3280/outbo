...
class UserProfile(BaseModel):
    """Extracted from the applicant's LinkedIn/resume for warm-path matching."""
...
async def extract_user_profile(
    ...
    """
    if not linkedin_url and not resume_url:
        return UserProfile()
...
async def _extract_with_llm(text: str, linkedin_url: str) -> UserProfile:
    ...
    prompt = f"""Extract the following from this person's profile/resume. Return JSON only.
...
If something is unclear or not mentioned, use an empty list.
...
{text[:6000]}"""
    ...
    try:
        ...
        logger.info(
            "User profile extracted: %d universities, %d previous companies, %d skills",
